...
async def main() -> None:
    ...
    agent = ReActAgent(
        llm=chat_model, tools=[OpenMeteoTool(), DuckDuckGoSearchTool(max_results=3)], memory= await load_memory(chat_model, conv_history)
    )
...
async def load_memory(chat_model: ChatModel, conv_history: SQLChatMessageHistory ):
    memory = SummarizeMemory(chat_model)
